...
uint8_t make_multiple_read(uint8_t * buf, int buf_len, uint8_t *id, uint8_t *data_address, uint8_t *data_length, uint8_t id_count)
{
	int i;
	add_header(buf);	
...
uint8_t make_same_multiple_read(uint8_t * buf, int buf_len, uint8_t *id, uint8_t data_address, uint8_t data_length, uint8_t id_count, int *answer_length)
{
	int i;
	add_header(buf);	
	buf[ID_POS] = BROADCAST_ID;
...
uint8_t make_same_addr_write(uint8_t * buf, int buf_len, uint8_t *id, uint8_t data_address, uint8_t data_length, uint8_t *data, uint8_t id_count, int *answer_length)
{
	int i;
	add_header(buf);	
	buf[ID_POS] = BROADCAST_ID;
...
uint8_t * validate_packet(dxl_packet *packet, uint8_t *buf, int buf_len)
{
	...
	while (buf < end && packet->state != 8)
	{
		...
		if (packet->state)
			++packet->state;
		//printf("state :%d last bytes: %d\n", packet->state, packet->left_bytes);
	}
	return buf;
}
