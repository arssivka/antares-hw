...
int bulk_write(int port_id, uint8_t addr, uint8_t data_length, uint8_t *device_ids, uint8_t *data, int devices) {
	...
	packet_length = make_same_addr_write(input_buffer[port_id], 255, device_ids, addr, data_length, data, devices, &answer_length);
...
int write_multiple_device(int broadcast, int port_id, uint8_t *device_ids, dxl_data *data, int devices) {
	if (!broadcast)
	{
		...
		{
			init_packet(&packets[port_id]);
			packet_length = make_write(input_buffer[port_id], 255, device_ids[i], data[device_ids[i]].addr, data[device_ids[i]].data, data[device_ids[i]].length);
			write_read_port(&controller, 
				port_id,
				input_buffer[port_id], 
				packet_length,
