...
void packet_parser_test() {
    uint8_t buf[3][3] = {{0x00, 0x00, 0xFF},
                         {0xFF, 0x01, 0x02},
                         {0x00, 0xFC, 0xFF}};
...
void test_dxl_controller() {
    ...
    register_port("/dev/ttyACM0", port_speed); // -> port id
    register_port("/dev/ttyACM1", port_speed);
    init_dxl_controller();
    add_dxl_device(0, 1); // <- port, servo
    add_dxl_device(0, 2);
    struct timespec start, end;
    while (1) {
        //clock_gettime(CLOCK_MONOTONIC_RAW, &start);
...
public:
    BaseMotionController(float min_angle, float max_angle, float start_angle)
            : min_angle(min_angle), max_angle(max_angle), start_angle(start_angle), current_angle(start_angle) {};
...
class LinearLoopedMotionController :
    ...
    float time;
public:
    LinearLoopedMotionController(float min_angle, float max_angle, float start_angle, float time = 1000,
                                 int direction = 1)
            : BaseMotionController(min_angle, max_angle, start_angle), time(time),
              direction(direction) { speed = (max_angle - min_angle) / time; };
